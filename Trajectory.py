...
# I don't know how, but this piece of shit is work
...
for i in range(path1.shape[0]):
    ...
    q, e = Tr_PTPSA(p_star, R_star, q_i)
...
for i in range(solution.shape[0]):
    points.append(ur_robot.fkine(solution[i, :6]).t)
...
ax.plot(path1[:, 0], path1[:, 1], path1[:, 2], 'g')
ax.plot(path2[:, 0], path2[:, 1], path2[:, 2], 'b')
ax.scatter(points[:, 0], points[:, 1], points[:, 2], 'm')
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('z')
plt.xlim([-0.2, 0.2])
